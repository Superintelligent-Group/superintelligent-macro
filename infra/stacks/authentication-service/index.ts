...
const GITHUB_CLIENT_ID = config.require(`github_client_id`);
...
const githubClientIdArn: pulumi.Output<string> = aws.secretsmanager
  .getSecretVersionOutput({ secretId: GITHUB_CLIENT_ID })
  .apply((secret) => secret.arn);
...
const secretKeyArns = [
  ...
  pulumi.interpolate`${githubClientIdArn}`,
